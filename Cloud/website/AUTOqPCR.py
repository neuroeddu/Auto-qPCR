...
def process_data(data, model, cgenes, cutoff, max_outliers, target_sorter=None, sample_sorter=None, csample=None):
	...
	if target_sorter is not None:
		# this list is case sensitive
		targets = [x.strip() for x in target_sorter.split()]
	# Add sorter to dataframe to order Target Names in output files
	sorter_index = dict(zip(targets, range(len(targets))))
	data['Target Order'] = data['Target Name'].map(sorter_index)

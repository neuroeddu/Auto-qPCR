...
def run_model(wdir, d, cfg):
    ...
    if h.verbosity == h.LOG_DEBUG:    
        h.bprint(s, 74)
        print(d1_1)
    d2 = d[f1].groupby(['Sample Name']).agg({'Quantity':'mean'})
    d2_1 = d[f1].groupby(['Sample Order','Sample Name']).agg({'Quantity':'mean'})    
    s = "Combined Endogenous Control Quantity Means and SSD"
    if h.verbosity == h.LOG_DEBUG:
        h.bprint(s, 74)
        print(d2_1)
